...
def process_directory():
    ...
    # Group building replacement files by their RMB base name
    building_replacements = {}
    for building_file in building_files:
        match = re.match(r"(.*\.RMB)-\d+-building(\d+)\.json", building_file)
        if match:
            full_name = match.group(1)
            base_name = full_name.split('.')[0]  # Extract base name
            index = int(match.group(2))
            building_replacements.setdefault(base_name, []).append((os.path.join(buildings_dir, building_file), index))

    # Apply replacements
    for rmb_file in rmb_files:
        # Extract the base name (first part before a period)
        base_name = rmb_file.split('.')[0]
        if base_name in building_replacements:
            for building_file, index in building_replacements[base_name]:
