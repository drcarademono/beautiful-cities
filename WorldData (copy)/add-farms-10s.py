...
def update_block_names(directory, block_name, used_prefixes):
    ...
    print(f"Searching for files with patterns: {pattern_farm} and {pattern_road} -> Found: {len(matching_files)} files")
    ...
    for json_file in json_files:
        ...
        for i, block_name in enumerate(block_names):
            ...
            if block_name == 'EMTYAA00.RMB':
                new_block_name, used_prefixes = update_block_names(directory, 'EMTYAA00.RMB', used_prefixes)
            elif block_name.startswith('WALLAA') and block_name.endswith('RMB'):
                index = block_name[6:8]
                if index.isdigit():
                    new_block_name, used_prefixes = update_block_names(directory, f'WALLAA{index}.RMB', used_prefixes)
                else:
                    continue
            else:
                continue
